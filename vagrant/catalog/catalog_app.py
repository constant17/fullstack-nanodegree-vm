...
def createUserWithPass():
    if request.method == 'POST':
        ...
        if not user_exist:
            n_user = User(name = request.form['name'], email=request.form['email'], passw = request.form['password'])
            session.add(n_user)
            session.commit()
...
def firstPage():
    ...
    if 'username' not in login_session:
        return render_template('p_index.html', categories= all_category)
    else:
        return render_template('index.html', categories= all_category) #items = all_items)
...
def editCategory(category_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('Category %s has been successfuly edited!' % editedCategory.name)
            return redirect(url_for('showCategories'))
    else:
